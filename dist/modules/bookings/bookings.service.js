...
const createBooking = async (bookingData, user_id) => {
...
const getBookings = async (user_id, role) => {
    if (role === "student") {
...
const getBookingsById = async (booking_id, role) => {
    if (role === "student") {
...
const payment = async (booking_id) => {
...
const sessionComplete = async (booking_id) => {
    return await prisma.$transaction(async (prisma) => {
        const booking = await prisma.booking.update({
            where: { id: booking_id },
